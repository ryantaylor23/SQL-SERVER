/* Ryan Taylor 
  ...
  3/7/2024 - VERSION 0.6 - Updated Project 3 to Project 4,
						   Created query to show all the disks in the database,
						   Created query to show all the borrowed disks and who borrowed them,
						   Created query to show the disks that have been borrowed more than once.

  3/8/2024 - VERSION 0.7 - Created query to show the disks that are on-loan and overdue and who took each disk,
						   Created a view that shows the borrowers who have not borrowed a disk,
						   Created a view as a subquery that shows the borrowers who have not borrowed a disk,
						   Created a query that shows the borrowers who have borrowed more than one disk.
...
--Query that is Displaying all of the disks in the database
SELECT DiskName, ReleaseDate, dt.Description AS 'Type', g.Description AS 'Genre', s.Description AS 'Status'
FROM Disk d
	LEFT JOIN DiskType dt
		ON d.DiskTypeID = dt.DiskTypeID
	LEFT JOIN Genre g
		ON d.GenreID = g.GenreID
	LEFT JOIN Status s
		ON d.StatusID = s.StatusID

--Query that Shows all borrowed disks and who borrowed them
SELECT b.LastName AS 'Last', b.FirstName AS 'First', d.DiskName AS 'Disk Name', BorrowedDate, ReturnedDate
FROM DiskHasBorrower dhb
	LEFT JOIN Borrower b
		ON dhb.BorrowerID = b.BorrowerID
	LEFT JOIN Disk d
		ON dhb.DiskID = d.DiskID

--Query that Shows the disks that have been borrowed more than once
SELECT d.DiskName AS 'Disk Name', COUNT(*) AS 'Times Borrowed'
FROM DiskHasBorrower dhb
	JOIN Disk d 
	ON dhb.DiskID = d.DiskID
GROUP BY DiskName
...
--Query that Shows the outstanding or on-loan and who took the disk
SELECT d.DiskName AS 'Disk Name', BorrowedDate AS 'Borrowed', s.Description AS 'Returned', b.LastName AS 'Last Name', b.FirstName AS 'First Name'
FROM DiskHasBorrower dhb
	JOIN Borrower b
		ON dhb.BorrowerID = b.BorrowerID
	JOIN Disk d
	...
	JOIN Status s
		ON d.StatusID = s.StatusID
WHERE s.Description = 'On Loan' OR s.Description = 'Overdue'
GO

--View that shows the borrowers who have not borrowed a disk 
CREATE VIEW View_Borrower_No_Loans_Join
AS
SELECT LastName, FirstName
FROM Borrower b
	LEFT JOIN DiskHasBorrower dhb
		ON b.BorrowerID = dhb.BorrowerID
WHERE BorrowedDate IS NULL
GO

--View that shows the borrowers who have not borrowed a disk but written as a subquery 
CREATE VIEW View_Borrower_No_Loans_SubQ
AS 
SELECT LastName, FirstName
FROM Borrower
WHERE BorrowerID NOT IN (SELECT BorrowerID FROM DiskHasBorrower)
GO

--Query that shows Borrowers who have borrowed more than one disk 
SELECT b.LastName AS 'Last Name', b.FirstName AS 'First Name', COUNT(dhb.BorrowerID) AS 'Books Borrowed'
FROM DiskHasBorrower dhb
	LEFT JOIN Borrower b
		ON dhb.BorrowerID = b.BorrowerID
GROUP BY b.LastName, b.FirstName
HAVING COUNT(dhb.BorrowerID) > 1 